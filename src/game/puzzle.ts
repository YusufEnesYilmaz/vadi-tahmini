...
/**
 * Ardışık şampiyon kuralı (kullanıcı kararı, 2026-07-21):
 * peş peşe gelen iki şampiyon AYNI koridor + AYNI cinsiyet olmasın.
 *
 * Neden bu ikisi: havuzun %39'u kadın ama Alt koridorluların %57'si kadın —
 * yani "kadın nişancı" kümelenmesi kadronun kendi yapısından geliyordu ve
 * oyuncuya rastgelelik bozukmuş gibi hissettiriyordu (ölçüldü, hata değildi).
 *
 * Kart ELENMEZ, ileri kaydırılır: her şampiyon tur içinde yine tam bir kez gelir.
 */
export function ayniKoridorVeCinsiyet(prevId: string, nextId: string): boolean {
  const a = byId(prevId)
  const b = byId(nextId)
  if (!a || !b) return false
...
export function nextPuzzle(top: TopMode, sub: PlaySub, filter: PoolFilter = ALL_FILTER): Puzzle {
  ...
  // Şampiyon modlarında ardışık "aynı koridor + aynı cinsiyet" engellenir
  const id = getDeck(`${sub}:${dk}`, pool, ayniKoridorVeCinsiyet).draw()
  const champion = byId(id)!
  if (sub === 'ability') {
