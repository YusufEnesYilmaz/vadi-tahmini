...
/**
 * Birden çok modun istatistiğini tek özete indirger — İstatistik ekranındaki
 * "özet kartı" bunu kullanır. Sayaçlar (played/won/…) TOPLANIR, seriler
 * (bestStreak/bestFirstTryStreak) modlar arası MAKSİMUM alınır (bir seri modun
 * kendi içinde tutulur; toplamı anlamsız olurdu). Saf fonksiyon — test edilebilir.
 */
...
export function aggregateStats(list: ModeStats[]): StatsSummary {
  return list.reduce<StatsSummary>(
    (a, s) => ({
      played: a.played + s.played,
      won: a.won + s.won,
      totalGuesses: a.totalGuesses + s.totalGuesses,
      firstTry: a.firstTry + s.firstTry,
      bestStreak: Math.max(a.bestStreak, s.bestStreak),
      totalScore: a.totalScore + s.totalScore,
    }),
    { played: 0, won: 0, totalGuesses: 0, firstTry: 0, bestStreak: 0, totalScore: 0 },
  )
}
...
function dayBefore(key: string): string {
  ...
  /*
   * YEREL bileşenlerden kur — `toISOString()` UTC'ye çevirir ve `todayKey()`
   * yerel tarih ürettiği için iki sistem karışırdı. UTC+13/+14 saat dilimlerinde
   * (NZ yaz saati, Samoa, Kiritimati) yerel öğlen bir önceki UTC gününe düşüyor,
   * dolayısıyla "dün" iki gün öncesi hesaplanıp seri haksız yere kırılırdı.
   */
